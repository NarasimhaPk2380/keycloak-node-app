...
var users_controller_1 = require("./api/controllers/users.controller");
...
describe("GET /api/books to handle the failure cases", function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, new okta_client_service_1.OktaClientService(users_controller_1.oktaClient, users_controller_1.oktaAuthClient).sessionLogin({ email: "test@gmail.com", password: "durga@2380" })];
                case 1:
                    session = _a.sent();
    ...
    it("should get error when session id is not provided", function (done) {
        supertest_1.default(server)
            .get("/api/books")
            .then(function (data) {
            expect(data.body).toEqual({
                statusCode: 400,
                message: "Please provide sessionid in the cookie",
            });
            done();
        });
    });
    it("should throw error when session id is invalid", function (done) {
        supertest_1.default(server)
            .get("/api/books")
            ...
            expect(data.body).toEqual({
                statusCode: 401,
                message: "Please login with the okta user, else register user",
            });
            done();
        });
    });
    it("should throw error when request body is invalid", function (done) {
        supertest_1.default(server)
            .post("/api/books")
            .send({})
            .set({ Cookie: "sessionid=" + sessionId })
            .then(function (data) {
            expect(data.body.message).toContain('Validation error: "name" is required');
            done();
        });
    });
